...
<div class="list__content">
    <div class="header__container">
        <h1 class="header__logo">{{ $user->name }}さんの勤怠</h1>
    </div>
    <div class="monthly__content">
        <form action="/admin/attendance/staff/{{ $user->id }}" method="get" class="month__nav">
    ...
    <div class="csv__export">
        <form action="/admin/attendance/staff/{{ $user->id }}/export" method="get">
            @csrf
            <input type="hidden" name="month" velue="{{ $currentMonth }}">
            <button type="submin" class="csv__btn">CSV出力</button>
        </form>
    </div>
</div>
@endsection